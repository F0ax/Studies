...
# AND
logging.info(byte1 & byte2)

# OR
logging.info(byte1 | byte2)

# XOR
logging.info(byte1 ^ byte3)

# Shifting right will lose the right-most bit
logging.info(byte2 >> 3)

# Shifting left will add a 0 bit on the right side
logging.info(byte2 << 1)

# See if a single bit is set
bit_mask = int('00000001', 2)  # Bit 1
logging.info(bit_mask & byte1)  # Is bit set in byte1?
logging.info(bit_mask & byte2)  # Is bit set in byte2?